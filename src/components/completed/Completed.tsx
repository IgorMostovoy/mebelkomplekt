...
const Completed = () => {
  ...
  const filteredKitchens = completedProjects.filter(
    ...
    <>
      <Box mb="160px" display="flex" flexDirection="column">
        ...
        <Flex display="flex" zIndex={2} ml="5%" mt="50px" mb="50px">
          {filteredKitchens.map((filteredKitchen) => (
            <CompletedList
              key={uniqid()}
              image={filteredKitchen.images[0].url}
              title={filteredKitchen.title}
              cost={filteredKitchen.cost}
            />
          ))}
        </Flex>
        ...
        <Box m="0 auto">
          <Link to="/completed">
            <Button variant="catalogButton">Смотреть портфолио</Button>
          </Link>
        </Box>
      </Box>
    </>
